...
def get_agg_df(df, agg_region):
    ...
    agg_df.set_index('operating_datetime_utc', inplace=True)
    agg_df.index = pd.to_datetime(agg_df.index)
    agg_df.dropna(subset=[agg_region], inplace=True)
    agg_df = agg_df.groupby([agg_df.index, agg_region]).sum()
    return agg_df
...
def get_diffs(df, agg_region):
    ...
    """
    df = df.reset_index().set_index(['operating_datetime_utc', agg_region]).sort_index()
